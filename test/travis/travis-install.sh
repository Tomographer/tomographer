...
if [ "$TT_PYTHON" == "system-python-2.7" ]; then
    ...
    export CMAKE_ADD_ARGS="$CMAKE_ADD_ARGS -DPYTHON_LIBRARY=$HOME/.miniconda/envs/test-environment/lib/libpython3.6m.so -DPYTHON_INCLUDE_DIR=$HOME/.miniconda/envs/test-environment/include/python3.6m -DZLIB_LIBRARY=/usr/lib/x86_64-linux-gnu/libz.so -DBOOST_LIBRARYDIR=/usr/lib/x86-64-linux-gnu/"
