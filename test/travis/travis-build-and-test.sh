...
$CMAKE_PATH/bin/cmake .. -DCMAKE_C_COMPILER=$CMAKE_C_COMPILER -DCMAKE_CXX_COMPILER=$CMAKE_CXX_COMPILER $CMAKE_ADD_ARGS -DBUILD_TOMOPY=on -DTOMOGRAPHER_ENABLE_TESTS=on -DTOMOGRAPHER_TEST_SETUPPY_BUILD=on -DEIGEN3_INCLUDE_DIR="$EIGEN_PATH" -DMATIO_INCLUDE_DIR="$MATIO_PREFIX_PATH/include" -DMATIO_LIBRARY="$MATIO_PREFIX_PATH/lib/libmatio.so" -DCMAKE_FIND_ROOT_PATH="$PYBIND11_PREFIX_PATH" -DPYTHON_EXECUTABLE=$PYTHON_EXECUTABLE  || cat build/CMakeFiles/CMakeError.log
