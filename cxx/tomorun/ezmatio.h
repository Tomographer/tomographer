...
#define MAT_SWITCH_COMPLEXABLE_TYPE(typ, ...)                           \
  ...
  // utilities to dump matrices and matrix lists, stored as std::vector<>

  template<class T>
  void dump_vector(const char * varname, const std::vector<T>& v,
                   const DimList& dims)
  {
    const unsigned int numel = get_numel(dims.begin(), dims.end());
    if (v.size() != numel) {
      std::cerr << "dump_c_var(\""<<varname<<"\"): Error: vector does not match given dims. size="
                << v.size() << ", prod(dims)=" << numel << std::endl;
      std::cerr << "\tdims=[";
      for (DimList::const_iterator it = dims.begin(); it != dims.end(); ++it) {
        if (it != dims.begin())
          std::cerr << ",";
        std::cerr << *it;
      }
    ...
    IndexList datavarindex(dims, 0);
    for (int k = 0; k < (int)v.size(); ++k) {
      std::cout << "\t"<<varname<<"(";
      for (IndexList::const_iterator it = datavarindex.begin(); it != datavarindex.end(); ++it) {
        if (it != datavarindex.begin())
          std::cout << ",";
        std::cout << *it;
      }
      std::cout << ") = " << v[k] << std::endl;
      datavarindex.increment(true);
    }
  }

  template<class T>
  void dump_vector_list(const char *varname, const std::vector<std::vector<T> >& v,
                        const DimList& innerdims)
  {
    char buf[128];
    for (unsigned int k = 0; k < v.size(); ++k) {
      snprintf(buf, 127, "%s[%d]", varname, k);
      dump_vector(buf, v[k], innerdims);
    }
  }



  // Utilities to dump std::vector<> natively into an ostream.


  // the purpose of those template classes was to be able to specialize it to different
  // contained types (in the std::vector<>, but it didn't work out so well (hell with
  // Eigen::Matrix<> template parameters, etc.)

  template<class It>
  struct VectorStreamerBase {
    static inline std::ostream& stream_vector(std::ostream& str, const It& begin, const It& end,
                                              const std::string& sepinline = ", ",
                                              const std::string& seplinebreak = " ;\n")
    {
      str << "[";
      const std::string * next_required_sep = NULL;
      for (It it = begin; it != end; ++it) {
        if (next_required_sep != 0) {
          str << *next_required_sep;
        }
        std::stringstream sstr;
        sstr << *it;
        const std::string itemstring = sstr.str();
        if (itemstring.find('\n') != std::string::npos) { // multiline
          if (it == begin) {
            str << "\n";
          }
          str <<  itemstring;
          next_required_sep = &seplinebreak;
        } else {
          str << itemstring;
          next_required_sep = &sepinline;
        }
      }
      if (next_required_sep == &seplinebreak) {
        str << "\n";
      }
      return str << "]";
    }
  ...
  template<class It>
  struct VectorStreamer : public VectorStreamerBase<It> { };

};


// override the actual << operator only if we were requested to do so explicitly with the
// EZMATIO_MORE_UTILS preprocessor define.

#ifdef EZMATIO_MORE_UTILS

template<class T>
std::ostream& operator<<(std::ostream& str, const std::vector<T>& vec)
{
  return MAT::VectorStreamer<typename std::vector<T>::const_iterator>::stream_vector(str, vec.begin(), vec.end());
}
