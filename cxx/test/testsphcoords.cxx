...
void test_coordtransform_and_jac()
{
  VectorXd cart(CART_DIM);
  cart.setOnes();

  VectorXd rtheta(CART_DIM);
  cart_to_sph(rtheta, cart); // cart -> rtheta

  cout << "cart = " << cart.transpose() << "\n--> rtheta = " << rtheta.transpose() << "\n";

  cart.setZero();
  sph_to_cart(cart, rtheta); // back to -> cart

  cout << "--> back to cart = " << cart.transpose() << "\n";

  // --------------------------------------------------

  cout << "More testing...\n";

  VectorXd mytheta(SPH_DIM);
  (mytheta << 1.34677, 0.835269 /*, 1.5708*/).finished();

  cout << "mytheta = " << mytheta.transpose() << "\n";

  VectorXd x2(CART_DIM);
  sphsurf_to_cart(x2, mytheta);

  cout << "--> x2 = " << x2.transpose() << "\n";

  VectorXd rthetaback(CART_DIM);
  cart_to_sph(rthetaback, x2);

  cout << "back to --> rthetaback = " << rthetaback.transpose() << "\n";
  ...
  for (k = 0; k < kmax; ++k) {
    ...
    rtheta.block(1,0,ds-1,1) = (rtheta.block(1,0,ds-1,1) + VectorXd::Constant(ds-1, 1.0)) * M_PI / 2; // theta_i in [0, pi] for 1 <= i < ds
    rtheta(ds) = (rtheta(ds) + 1) * M_PI; // theta_{ds} in [0, 2*pi]

    //std::cout << rtheta.transpose() << "\n";

    vol += cart_to_sph_jacobian(rtheta);
  }
  ...
  vol *=  1.0 * 2*M_PI *  VectorXd::Constant(ds-1, M_PI).array().prod();
  ...
  for (k = 0; k < kmax; ++k) {
    ...
    theta.setRandom();
    // so translate them to the correct ranges. Remember:  rtheta(i) + 1  is in the interval (0, 2)
    theta.block(0,0,ds-1,1) = (theta.block(0,0,ds-1,1) + VectorXd::Constant(ds-1, 1.0)) * M_PI / 2; // theta_i in [0, pi] for 1 <= i < ds
    theta(ds-1) = (rtheta(ds-1) + 1) * M_PI; // theta_{ds} in [0, 2*pi]

    //std::cout << theta.transpose() << "\n";

    surf += surf_sph_jacobian(theta);
  }
  ...
  // multiply by volume of parameter space
  surf *=  2*M_PI *  VectorXd::Constant(ds-1, M_PI).array().prod();
...
struct sphsurf_to_cart_fn {
  ...
  void operator()(MatrixBase<Der2>& cart, const MatrixBase<Der1>& theta) {
    sphsurf_to_cart(cart, theta);
  }
};
...
struct sphsurf_to_diffcart_fn {
  template<typename Der1, typename Der2>
  void operator()(MatrixBase<Der2>& dxdthetalinear, const MatrixBase<Der1>& theta) {
    Array<double, N, DS> dxdtheta;
    //cout << "start fn eval\n";
    sphsurf_diffjac(dxdtheta, theta);
    //cout << "mid fn eval, dxdtheta's shape is (rows="<<dxdtheta.rows()<<",cols="<<dxdtheta.cols()<<")\n";
    for (int i = 0; i < DS; ++i) {
...
#define DEF_N 10
#define DEF_DS (DEF_N-1)

void test_jacdiff()
{
  // Choose a random point, and test the derivatives there
  Matrix<double, DEF_DS, 1> theta;
  theta.setRandom();
  // but we need it on the surface of the sphere
  theta /= theta.norm();
  //theta << M_PI/2, M_PI;

  Array<double, DEF_N, DEF_DS> dxdtheta;
  sphsurf_diffjac(dxdtheta, theta);

  cout << "dxdtheta at theta=\t"<<theta.transpose() <<":\n"<<dxdtheta << "\n";

  Matrix<double, DEF_N, 1> x;
  sphsurf_to_cart(x, theta);
  cout << "x(theta) = "<< x.transpose()<< "\n";

  check_derivatives(dxdtheta, // derivatives
                    theta, // point
  ...
  Array<double, DEF_N, DEF_DS*DEF_DS> ddxddtheta;
  sphsurf_diffjac2(ddxddtheta, theta);

  Array<double, DEF_N*DEF_DS, DEF_DS> ddxddtheta_reshaped;
  ...
  cout << "about to check second derivatives\n";

  check_derivatives(ddxddtheta_reshaped, // derivatives of the derivatives :)
                    theta, // point
